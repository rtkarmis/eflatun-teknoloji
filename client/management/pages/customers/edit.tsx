...
import { UpdateCustomerCommandRequest } from '@/types/customer-dto';
import { Card, CardHeader, CardTitle, CardContent } from '../../components/ui/card';

type CustomerFormValues = {
  request:UpdateCustomerCommandRequest
};
...
export default function EditCustomer() {
  ...
  const { updateCustomer, getCustomerById } = useCustomerService();
  ...
  const {
    ...
    queryFn: async () => {
      if (!id) return null;
      const res = await getCustomerById(String(id));
  ...
  const onSubmit = async (values: CustomerFormValues) => {
    const res = await updateCustomer(String(id), values.request);
    if (res && res.success) {
      // page ve pageSize varsa geri dönerken koru
      if (page || pageSize) {
        router.push({
          pathname: '/customers',
