...
import { CreateSupplierOrderCommandRequest } from "@/types/order-dto";
import { Card, CardHeader, CardTitle, CardContent } from "@/components/ui/card";

type OrderFormValues = {
  request: CreateSupplierOrderCommandRequest;
};

export default function AddSupplierOrder() {
  ...
  const { createSupplierOrder } = useSupplierService();

  useEffect(() => {
    (async () => {
      const productsRes = await getProducts();
      setProducts(
        productsRes && productsRes.success && productsRes.data
          ? productsRes.data
          : []
      );
    })();
  ...
  const onSubmit = async (values: OrderFormValues) => {
    const res = await createSupplierOrder(String(id), values.request);
    if (res && res.success) {
      queryClient.invalidateQueries({ queryKey: ["supplierOrders", id] });
      ...
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-5">
            {/* Çoklu ürün ve adet seçimi, mobilde dikey, masaüstünde yatay */}
            ...
            <Button
              ...
              disabled={form.watch("request.orderItems").some(item => {
                ...
                return selectedProduct && (selectedProduct.stock == null || Number(selectedProduct.stock) <= 0);
              })}
              onClick={() =>
